...
import PopupMenu from "../InterviewDashboard/PopupMenu";
import VideoPlayer from "../InterviewDashboard/VideoPlayer";
import ControlsBar from "../InterviewDashboard/ControlsBar";
import NotesSection from "../InterviewDashboard/NotesSection";
import ExaminerNotesSection from "../InterviewDashboard/ExaminerNotesSection";
import NavigationButtons from "../InterviewDashboard/NavigationButtons";
import pauseVideoUrl from "../../assets/pause.mp4";

const CalculationDashboard = () => {
  ...
  const [currentStation, setCurrentStation] = useState("Concentration 1");
  ...
  // New state for Calculation and Data
  ...
  const pauseSegmentRef = useRef(null);
  const currentScenarioRef = useRef(null);

  // Add sections data for Calculation and Data
  const sections = [
    "Concentration 1",
    "Concentration 2",
    "Conversion",
  ...
  // Initialize Calculation and Data on load
  useEffect(() => {
    console.log("🔧 Calculation and Data useEffect triggered");

    const initializeCalculationData = async () => {
      try {
        console.log("Initializing Calculation and Data...");
        ...
        // Use selected scenario or default to "Concentration 1" for initial load only
        const initialScenario = selectedSection || "Concentration 1";
        console.log("Using initial scenario:", initialScenario);

        const response = await APIService.getCalculationData({
          scenario: initialScenario,
          start: "true",
        });

        console.log("Calculation and Data Response:", response);
        ...
        console.error("Error initializing Calculation and Data:", error);
    ...
    if (!calculationData && !videoUrlRef.current) {
      console.log("🎯 Starting Calculation and Data initialization");
      initializeCalculationData();
    } else {
      console.log("🔄 Calculation and Data already initialized, skipping");
    }
  ...
  useEffect(() => {
    const initializeAudio = async () => {
      try {
        ...
        console.log(
          "Initializing audio for both recording and voice detection..."
        );
        ...
        recorder.onstop = () => {
          console.log(
            "📼 MediaRecorder stopped, processing audio chunks:",
          ...
          audioChunks.current = [];
          console.log(
            "📼 Audio recording processed, blob size:",
            audioBlob.size
          );
        };
    ...
    return () => {
      if (
        window.location.pathname === "/calculation-interview" &&
  ...
  const loadNextQuestion = async () => {
    ...
    try {
      ...
      const currentScenario =
        currentScenarioRef.current || selectedSection || "Concentration 1";

      console.log("📡 Calling API with scenario:", currentScenario);
      ...
      if (response.question) {
        ...
        // Use video data from the API response for the current station
        ...
        if (mainVideoRef.current) {
          const [minutes, seconds] = response.question.startTimestamp
            .split(":")
            .map(Number);
          const startTimeInSeconds = minutes * 60 + seconds;

          console.log(
            "📹 Setting video to timestamp:",
            response.question.startTimestamp
          );
  ...
  const stopAudioRecording = () => {
    ...
    if (
      ...
      console.log(
        "❌ Cannot stop recording - recorder unavailable or not recording"
      );
      return;
    }
  ...
  const startSessionRecording = async () => {
    try {
      ...
      sessionRecorderRef.current.onstop = () => {
        ...
        a.href = url;
        a.download = "calculation-interview-session.webm";
        a.click();
      };
    ...
    if (currentVideoState?.pauseSegment && !currentVideoState.isPauseSegment) {
      ...
      try {
        ...
        if (mainVideoRef.current) {
          ...
          setIsMainVideoPlaying(true);
          console.log(
            "✅ Pause segment should be playing, voice detection active"
          );

          // Ensure checkAudio is running for the new pause segment
          setTimeout(() => {
            console.log(
              "🔄 Ensuring voice detection continues after pause segment switch..."
            );
  ...
  const checkAudio = () => {
    try {
      ...
      if (!analyserRef.current) {
        console.log(
          "🚨 Analyser missing - voice detection broken, need to restart audio"
        );
      ...
      if (!videoState.isPauseSegment) {
        ...
        if (hasSpoken.current || silenceCount.current > 0) {
          console.log(
            "🔄 Resetting voice detection state (not in pause segment)"
          );
          hasSpoken.current = false;
          silenceCount.current = 0;
        }
      ...
      if (
        Math.floor(Date.now() / 2000) !== Math.floor((Date.now() - 16) / 2000)
      ) {
      ...
      if (average > 10) {
        ...
        if (silenceCount.current >= 300) {
          // 5 seconds
          console.log(
            "✅ 5 seconds of silence completed, stopping recording and loading next video"
          );
          console.log(
            "✅ Current scenario before loading next:",
          ...
          setTimeout(() => {
            console.log(
              "🔄 Calling loadNextQuestion after audio processing delay..."
            );
            console.log(
              "🔄 Scenario at loadNextQuestion call:",
              currentScenarioRef.current
            );
            loadNextQuestion();
          }, 1000);
    ...
    <div className="h-full bg-background p-6 interview-container relative">
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black/70">
          <div className="bg-white rounded-2xl p-8 max-w-md text-center shadow-2xl animate-fade-up">
            ...
            <p className="text-gray-700 mb-6">
              Your calculation interview session has ended.
            </p>
            <button
      ...
      <div className="animate-fade-up mb-6">
        <PopupMenu
          ...
          professionalJudgementData={calculationData}
          setProfessionalJudgementData={setCalculationData}
          setCurrentQuestionId={() => {}} // Not needed for calculation data
      ...
      <NotesSection
        ...
        candidatePrompt={null} // Not applicable for calculation data
        background={null} // Not applicable for calculation data
